	...
	public class Filters
	{
		...
		public static Bitmap Dither(Bitmap source, PixelFormat format, DitherMethod method)
		{
			switch (method)
			{
				...
				default:
					return NoDither(source, format);
			}
		}

		private static Bitmap NoDither(Bitmap source, PixelFormat format)
		{
			var result = new PrecisionBitmap(source);
		...
		private static Bitmap Burkes(Bitmap source, PixelFormat format)
		{
		...
		private static Bitmap Sierra3(Bitmap source, PixelFormat format)
		{
			var result = new PrecisionBitmap(TileBuffer(source));
		...
		private static Bitmap Sierra2(Bitmap source, PixelFormat format)
		{
			var result = new PrecisionBitmap(TileBuffer(source));
		...
		private static Bitmap FilterLite(Bitmap source, PixelFormat format)
		{
			var result = new PrecisionBitmap(TileBuffer(source));
		...
		private static int GetActualPosition(int pos, int max)
		{
			if (pos < 0)
			{
				return max + pos;
		...
		private static PrecisionColor ApproximatePixel(PrecisionColor color, PixelFormat format)
		{
			int rM;
			int gM;
			int bM;
		...
		private static Bitmap TileBuffer(Bitmap source)
		{
		...
		private static Bitmap TileDebuffer(Bitmap source)
		{
			var result = new Bitmap(source.Width - 10, source.Height - 10);
			result.SetResolution(source.HorizontalResolution, source.VerticalResolution);
